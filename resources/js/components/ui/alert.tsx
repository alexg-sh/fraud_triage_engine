...
const alertVariants = cva(
  "group/alert relative grid w-full gap-1 rounded-2xl border border-border/70 px-4 py-4 text-left text-sm shadow-lg shadow-black/5 has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-20 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-3 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current *:[svg:not([class*='size-'])]:size-5",
  {
    variants: {
      variant: {
        default: "bg-card/90 text-card-foreground backdrop-blur-sm",
        destructive:
